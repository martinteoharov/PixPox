...
use crate::{
    geometry::{attribute::Attribute, geometry::Geometry, index::Index},
    material::material::Material,
    renderer::{
        wgpu_attributes::WGPUAttributes, wgpu_bindings::WGPUBindings, wgpu_indices::WGPUIndices,
        wgpu_render_pipeline::WGPURenderPipelines, wgpu_samplers::WGPUSamplers,
        wgpu_textures::WGPUTextures,
    },
    resource::resource::{ResourceId, ResourcePools},
    scene::{camera::PerspectiveCamera, mesh::Mesh, node::Node, scene::Scene},
};
...
impl Default for WGPURendererOptions {
    fn default() -> Self {
        WGPURendererOptions { sample_count: 4 }
    }
}
...
impl WGPURenderer {
    ...
    fn update(
        ...
        for node_rid in scene.collect_nodes(pools).iter() {
            ...
            self.textures
                .update_from_material(&self.device, &self.queue, pools, material);

            self.samplers
                .update_from_material(&self.device, pools, material);
    ...
    fn render_internal(&self, pools: &ResourcePools, scene_rid: &ResourceId<Scene>) {
